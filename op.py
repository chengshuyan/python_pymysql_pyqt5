...
class Ui_Form(object):
    def setupUi(self, Form):
        ...
        self.model.setHorizontalHeaderLabels(
            ['航班号', '起点', '终点', '日期', '起飞时刻', '到达时刻', '剩余座位数', '票价', '折扣票价', '折扣率', '航班所属航班公司'])
        #********获得数据
        self.tableView.setModel(self.model)
        self.pushButton_3 = QtWidgets.QPushButton(Form)
        self.pushButton_3.setGeometry(QtCore.QRect(330, 10, 31, 21))
        self.pushButton_3.setObjectName("pushButton_3")
        self.pushButton_3.clicked.connect(self.button_add)
        self.pushButton_4 = QtWidgets.QPushButton(Form)
        self.pushButton_4.setGeometry(QtCore.QRect(360, 10, 31, 21))
        self.pushButton_4.setObjectName("pushButton_4")
        self.pushButton_4.clicked.connect(self.button_del)
    ...
    def button_add(self):
        ...
        # 根据数据对票务查询
        # 使用游标对象执行SQL语句；
        sql1 = 'INSERT INTO F_S (航班号,日期) VALUES(\''+ number + '\',' +'\'' + dataTime + '\')'
        db = pymysql.connect(host='localhost', user=Dialog.UserName, password=Dialog.PassWord, port=3306,
                             db='SQLB17121207',
                             charset='utf8')
        # 开启mysql的游标功能，创建一个游标对象；
        cursor = db.cursor()
    ...
    def button_del(self):
        ...
        # 根据数据对票务查询
        # 使用游标对象执行SQL语句；
        sql1 = '''DELETE FROM F_S WHERE 航班号 = \'''' + number + '''\' AND 日期 = \'''' + dataTime + '\''
        db = pymysql.connect(host='localhost', user=Dialog.UserName, password=Dialog.PassWord, port=3306,
                             db='SQLB17121207',
                             charset='utf8')
        # 开启mysql的游标功能，创建一个游标对象；
